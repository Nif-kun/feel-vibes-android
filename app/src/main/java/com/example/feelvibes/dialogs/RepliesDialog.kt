...
import com.example.feelvibes.databinding.RepliesDialogBinding
...
class RepliesDialog(
    ...
    var userId:String?,
    private val replies: ArrayList<ReplyModel>
) : FragmentBottomSheetDialogBind<RepliesDialogBinding>(RepliesDialogBinding::inflate), CommentRecyclerEvent {

    private lateinit var accountViewModel : AccountViewModel

    var onDismissListener : (() -> Unit)? = null
    var onQueryListener : ((ArrayList<ReplyModel>) -> Unit)? = null
    ...
    private fun setupRecyclerAdapter() {
        // TODO make a ReplyRecyclerAdapter as there are problems with using the existing one.
        binding.commentsRecyclerView.adapter = CommentRecyclerAdapter(
            ...
            postId,
            userId,
            replies.filterIsInstanceTo(arrayListOf())
        )
    }
    ...
    private fun loadUser() {
        if (userId != null) {
            FVFireStoreHandler.getProfilePicture(userId!!) { uri ->
                if (uri != null)
                    Glide.with(this).load(uri).into(binding.profilePicUser)
            }
        }
    }
        ...
        binding.commentBtn.setOnClickListener {
            if (userId != null) {
                ...
                keyboardEditableDialog.onSendListener = { comment ->
                    if (comment != null) {
                        FVFireStoreHandler.addCommentReply(ownerId, postId, commentId, userId!!, comment) { success, model, exception ->
    ...
    override fun onCommentDelete(commentModel: CommentModel) {
        replies.remove(commentModel as ReplyModel)
        modified = true
        setupRecyclerAdapter()
    }

    override fun onCommentLiked(commentModel: CommentModel, userId: String) {
        replies.find { it.id == commentModel.id }?.likes?.add(userId)
        modified = true
    }

    override fun onCommentUnliked(commentModel: CommentModel, userId: String) {
        replies.find { it.id == commentModel.id }?.likes?.remove(userId)
        modified = true
    }
    ...
    override fun onDismiss(dialog: DialogInterface) {
        ...
        if (modified)
            onQueryListener?.invoke(replies)
